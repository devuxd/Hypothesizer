...
function Todo({ todo, index, completeTodo, unCompleteTodo, removeTodo, editTodo, setTodo, editing }) {
  // BUG IS HERE
  var booleanValue = todo.completed && index
  if (!editing)
    return (
      <div id="todo" className="todo block text--700 text-center bg-gray-200 hover:bg-gray-400 px-4 py-2 mt-2" style={{ backgroundColor: todo.completed ? "#bdffcf" : "" }}>
        {todo.text}
        <div id="NotTodo">
          {booleanValue ? <button onClick={() => unCompleteTodo(index)} className="bg-gray-300 hover:bg-yellow-400 text-gray-800 font-bold py-2 px-4 rounded inline-flex items-center transition duration-500 ease-in-out transform hover:-translate-y-1"> Pending </button>
