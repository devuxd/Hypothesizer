...
function Todo({ todo, index, completeTodo, unCompleteTodo, removeTodo, editTodo, setTodo, editing }) {
  if (!editing)
    return (
      // BUG IS HERE
      <div id="todo" className="todo block text--700 text-center bg-gray-200 hover:bg-gray-400 px-4 py-2 mt-2" style={{ backgroundColor: "#bdffcf" }}>
        {todo.text}
        <div id="NotTodo">
          {/* BUG IS HERE */}
           <button onClick={() => unCompleteTodo(index)} className="bg-gray-300 hover:bg-yellow-400 text-gray-800 font-bold py-2 px-4 rounded inline-flex items-center transition duration-500 ease-in-out transform hover:-translate-y-1"> Pending </button>
